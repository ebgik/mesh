...
exports.getUserInfo = function(id,callback){
	var query = db.query('SELECT name,image,id FROM users WHERE id = ?',[id],function(err,result){
		if (err||!result[0])
			callback('error');
...
exports.getSocket = function(id,callback){
	var query = db.query('SELECT socket_id FROM sessions WHERE user_id = ?',[id],
		function(err,result){
			...
			{
				if (result[0]) callback(result[0].socket_id)
				else callback('none');
			}
		})
}
...
exports.searchUsers = function(str,callback){
	var query = db.query('SELECT id,name,image FROM users WHERE name LIKE "%'+str+'%"',
		function(err,result){
			if (err) callback('error')
			else
			{
