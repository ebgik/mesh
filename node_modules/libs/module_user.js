...
exports.addUser = function(name,email,pass,callback){
	var pass = crypto.createHash('md5').update(pass).digest("hex");
	var query = db.query('INSERT INTO users SET name = ?, email = ?, pass = ?',[name,email,pass],
		function(err,result){
			if (err)
			{
			  	callback('error');
...
exports.uploadAvatar = function(id_user,image,callback){
	var query = db.query('UPDATE users SET image = ? WHERE id = ?',[image,id_user],function(err,result){
