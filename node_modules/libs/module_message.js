...
exports.getDialogues = function(id_user,callback){
	var query = db.query('SELECT D.*, U.id, U.name, U.image FROM dialogues AS D, users AS U WHERE (D.id_sender = ? AND D.id_sel = U.id) OR (D.id_sel = ? AND D.id_sender = U.id) ORDER BY date_update DESC',
		[id_user,id_user],
		function(err,result){
			if (result.length>0)
			{
