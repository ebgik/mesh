		  		...
		  		[id_sender,id_sel,id_sel,id_sender],function(errDia,resultDia){
		  			...
		  			else
		  			{
		  				if (resultDia.length>0)
		  					var query3 = db.query('UPDATE dialogues SET id_last_mess = ?,date_update = ? WHERE (id_sender = ? AND id_sel = ?) OR (id_sender = ? AND id_sel = ?)',
		  						[result.insertId,data,id_sender,id_sel,id_sel,id_sender],function(errDialog,resultDialog){
		  							if (errDialog) callback(errDialog)
		  							else callback({ result:'success', date : moment(now).format('LLL')});
		  						})	
		  				else
		  					var query3 = db.query('INSERT INTO dialogues SET id_sender = ?, id_sel = ?, id_last_mess = ?, date_update = ?',
		  						[id_sender,id_sel,result.insertId,data],function(errDialog,resultDialog){
		  							if (errDialog) callback(errDialog)
		  							else callback({ result:'success', date : moment(now).format('LLL')});
		  						})
		  			}
...
exports.getMessages = function(user,sel,callback){
	...
	var query = db.query('SELECT M.*, U.name, U.family, U.image FROM message AS M, users AS U WHERE ((M.id_sender = ? AND M.id_sel = ?) OR (M.id_sender = ? AND M.id_sel = ?)) AND M.id_sender = U.id ORDER BY M.id',
		[user,sel,sel,user],
		function(err,result){
			//console.log(result)
			if (result[0])
...
exports.getDialogues = function(id_user,callback){
	var query = db.query('SELECT D.*, U.id, U.name, U.family, U.image FROM dialogues AS D, users AS U WHERE (D.id_sender = ? AND D.id_sel = U.id) OR (D.id_sel = ? AND D.id_sender = U.id) ORDER BY date_update DESC',
		[id_user,id_user],
		function(err,result){
			if (result.length>0)
			{
				result.forEach(function(item,i,arr){
					var query2 = db.query('SELECT M.*, U.image FROM message AS M, users AS U WHERE M.id = ? AND M.id_sender = U.id',[item.id_last_mess],
